...
//	리스트 페이지 보여줌 (get , post 방식 전부 여기로 들어옴)
	...
	public String list(Board board , HttpServletRequest request , Model model) {
		...
		try {
			...
			int currentPage = 1; //현재 페이지 번호(디폴트 페이지 번호)
			
			if(pageNum != null) currentPage = Integer.parseInt(pageNum);
			
			
			String searchKey = request.getParameter("searchKey"); //검색 키워드 (subject ,content ,name 중 택1)
			...
			if(searchValue == null) {
				...
				if(request.getMethod().equalsIgnoreCase("GET")) {
					...
					//쿼리 파라미터의 값(searchValue)을 디코딩 해준다.
